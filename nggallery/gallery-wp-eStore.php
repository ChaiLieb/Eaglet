...
<?php echo get_the_term_list($post->ID, 'photographer', '<h4 class="subtext">Photographer: ', ', ', '</h4>'); ?>
<a href="<?php the_permalink() ?>" title="<?php the_title(); ?>"><?php the_title('<h1 class="albumtitle">', '</h1>'); ?></a>
...
<div class="ngg-galleryoverview" id="ngg-gallery-<?php echo $gallery->ID ?>">
	...
	<div id="ngg-image-<?php echo $image->pid ?>" class="ngg-gallery-thumbnail-box" <?php echo $gallery->imagewidth ?> >
		<div class="ngg-gallery-thumbnail" >
			...
			<span><?php echo $image->caption ?></a>
<?php
global $wp_query;
$postid = $wp_query->post->ID;
echo get_post_meta($postid, 'button', true);
wp_reset_query();
?></span>
		</div>
	</div>
